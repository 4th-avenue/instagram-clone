<div wire:key="comment-{{ $comment->id }}" class="flex items-center gap-3 py-2">
    <x-avatar wire:ignore src="https://source.unsplash.com/500x500?face-{{rand(1,10)}}" class="size-9 mb-auto" />
    ...
    <div class="grid grid-cols-7 w-full gap-2">
        ...
        <div class="col-span-7 flex gap-2 text-sm items-center text-gray-700">
            ...
            <span class="font-bold">
                @if ($comment->totalLikers > 0)
                    <p class="font-bold text-sm">{{ $comment->totalLikers }} {{ $comment->totalLikers > 1 ? 'likes':'like' }}</p>
                @endif
            </span>
