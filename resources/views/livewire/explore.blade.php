<div class="my-5 lg:px-8">
    <ul class="grid grid-cols-3 sm:gap-3">
        ...
        <li onclick="Livewire.dispatch('openModal', {component:'post.view.modal', arguments:{'post':{{ $post->id }}}})" class="h-28 sm:h-80 w-full cursor-pointer border rounded bg-black relative items-center flex justify-center group">
            ...
            @if ($post->type == 'post' && $post->media->count() > 1)
                <div class="absolute top-4 right-4 z-10 text-white">
                    <span>
                        <svg class="size-6" fill="none" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
                            <g fill="currentColor">
